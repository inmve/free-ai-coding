...
多くのAIコーディングツールは「無料」と謳っていますが、プロフェッショナルグレードモデルへのアクセスは通常すぐに制限に達し、その後グレードダウンされます。各ツールは異なる制限（クレジット、トークン、リクエスト）を使用しているため、公正な比較は困難です。このリストでは、それらを並べて比較し、**プロフェッショナルグレードLLMで実際に何時間のコーディングを無料で得られるか**を示しています。

## TL;DR — プロフェッショナルグレードコーディング用無料ティアランキング
_（最も寛大なものから最も制限的なものまで順位付け；時間推定は以下の[方法論](#方法論--前提条件)を使用）_

| ツール | プロフェッショナルグレードモデル | 無料ティア制限 | おおよその無料コーディング時間 | クレジットカード |
|--------|----------------------------------|----------------|--------------------------------|------------------|
| [Qwen Code](#qwen-code) | Qwen3-Coder-480B | 2,000リクエスト/日 | 〜33時間/日（〜1000時間/月） | 不要 |
| [Rovo Dev CLI](#rovo-dev-cli) | Claude Sonnet 4 | 5Mトークン/日（ベータ） | 〜20時間/日（〜600時間/月） | 不要 |
| [Gemini CLI](#gemini-cli) | Gemini 2.5 Pro | 100リクエスト/日 | 〜1.7時間/日（〜50時間/月） | 不要 |
| [Kilo Code](#kilo-code) | Claude Opus/Sonnet, Gemini 2.5 Pro, GPT‑4.1 | $20登録クレジット（一回限り） | 〜3時間合計 | 必要 |
| [Warp](#warp) | Claude, OpenAI, Gemini | 150リクエスト/月 | 〜2.5時間/月 | 不要 |
| [Amazon Q Developer](#amazon-q-developer) | Claude Sonnet 4 | 50エージェントチャット/月 | 〜0.8時間/月 | 必要 |
| [GitHub Copilot](#github-copilot) | GPT‑4o, Claude 3.5 Sonnet, Gemini 2.0 Flash | 50チャット + 2,000補完/月 | 〜0.8時間/月（エージェント） | 不要 |
| [Windsurf](#windsurf) | OpenAI, Anthropic, Google, xAI | 25クレジット/月 | 〜0.4時間/月 | 必要 |
| [AWS Kiro](#aws-kiro) | Claude Sonnet 4 / 3.7 | 日次制限（非公開） | 不明 | 不要（プレビュー） |

## これらの推定はどの程度正確ですか？

これらの時間推定は計算に基づいていますが、**実際の使用量はコーディングスタイルとタスクの複雑さによって異なります**。[実際のコーディング時間を共有して→](https://free-ai-usage.vercel.app/)このリソースをより正確にするのを手伝ってください
...
> **〜0.4時間/月 OpenAI、Anthropic、Google、xAIモデル全体で**
- 25プロンプトクレジット/月制限
...
- **目標**：フロンティアモデルへのアクセスによりエージェンティックコーディングシステムを比較する。
- **モデルが「プロフェッショナルグレード」として適格な条件は？** この比較では、モデルはSWE-bench Verifiedで≥60%を達成し、実世界ソフトウェア工学能力を実証する必要があります。現在適格なモデル：GPT-5（74.9%）、Claude Opus 4.1（74.5%）、Claude Sonnet 4（72.7%）、GPT-5 mini（71.0%）、Qwen3-Coder-480B（69.6%）、Gemini 2.5 Pro（63.2%）。
- **[1] リクエストから時間へ**：60リクエスト ≈ AI支援コーディング1時間（実世界タスクシミュレーションに基づく）。
- **[2] トークンから時間へ**：〜25万トークン ≈ コーディング1時間（実世界AIエージェント使用データに基づく）。
- **[3] チャット/クレジットから時間へ**：マルチターンエージェンティックチャットとプロンプトクレジットは推定目的で単一リクエストと同等と仮定。
- エラーや不足しているソースリンクを見つけた場合は、issueやpull requestを開いてください。
